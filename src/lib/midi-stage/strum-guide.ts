...
/** Keep the current unplayed strum until its scoring window closes.
 * Both time and the active Judge's late window are in song-time seconds.
 */
export function nextStrum(
  song: Pick<Song, "beats" | "bpm">,
  notes: readonly Pick<ChartNote, "time" | "state">[],
  time: number,
  ...
  while (lo < hi) {
    const mid = (lo + hi) >>> 1;
    // Match Judge.tick's inclusive late boundary, including float tolerance.
    if (notes[mid]!.time < time - lateWindowSeconds - 1e-8) lo = mid + 1;
    else hi = mid;
  }
